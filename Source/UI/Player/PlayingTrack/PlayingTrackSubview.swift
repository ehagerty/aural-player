 ...
 */
@IBDesignable
class PlayingTrackSubview: NSView {
    ...
    func showView(controlsView: NSView, functionsView: NSView) {

        DispatchQueue.main.async {

            if !controlsView.isDescendant(of: self.controlsBox) {
                self.controlsBox.addSubview(controlsView)
                self.controlsBox.bringToFront()
            }

            if !functionsView.isDescendant(of: self.functionsBox) {
                self.functionsBox.addSubview(functionsView)
            }
        }
